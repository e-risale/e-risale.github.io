                            ...
                            return (
                                <div key={comment.id} className={`p-5 rounded-2xl border transition-all ${darkMode ? 'bg-[#25262b] border-gray-700' : 'bg-white border-[#e6e0d2]'}`}>
                                    <div className="flex items-center gap-3 mb-3">
                                        ...
                                        <div>
                                            <div className={`font-bold text-sm ${darkMode ? 'text-gray-200' : 'text-gray-900'}`}>{comment.name || 'Misafir'}</div>
                                    ...
                                    <div className="flex items-center justify-between border-t border-dashed border-gray-500/20 pt-3">
                                        <button
                                            ...
                                            className={`text-xs font-bold uppercase tracking-wider hover:underline transition-colors ${replyToCommentId === comment.id ? 'text-amber-600' : 'opacity-60'}`}
                                        >
                                            {replyToCommentId === comment.id ? 'İptal' : 'Yanıtla'}
                                        </button>
                                        ...
                                        <div className={`mt-6 pt-2 pl-4 border-l-2 ${darkMode ? 'border-gray-700' : 'border-amber-200'}`}>
                                            ...
                                            <div className="space-y-3">
                                                {displayedReplies.map(reply => {
                                                    const isEditor = reply.name && (reply.name === 'Editör' || reply.name.includes('Editör'));
                                                    return (
                                                        <div key={reply.id} className={`p-3 rounded-xl border ${isEditor ? (darkMode ? 'bg-indigo-900/20 border-indigo-500/50' : 'bg-amber-50 border-amber-200 shadow-sm') : (darkMode ? 'bg-[#1a1b1e] border-gray-800' : 'bg-gray-50 border-gray-200')}`}>
                                                            <div className="flex items-center gap-2 mb-1">
                                                                ...
                                                                <span className={`font-bold text-xs ${isEditor ? (darkMode ? 'text-indigo-400' : 'text-amber-700') : (darkMode ? 'text-gray-300' : 'text-gray-900')}`}>
                                                                    {reply.name}
                                                                </span>
                                                            ...
                                                            <div className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-700'} ${isEditor ? 'italic' : ''} pl-8`}>{reply.feedback}</div>
                                                        </div>
                                                    );
                                                })}
                                            </div>
