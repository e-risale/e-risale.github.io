...
const FeedbackModal = ({
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 animate-in fade-in" onClick={onClose}>
            <div className={`w-full max-w-lg rounded-2xl shadow-2xl p-6 ${darkMode ? 'bg-[#1a1b1e] text-gray-200' : 'bg-white text-gray-800'}`} onClick={e => e.stopPropagation()}>
                ...
                {!user ? (
                    ...
                    <>
                        {selectedText && (
                            <div className={`p-3 rounded-lg mb-4 text-sm italic border-l-4 ${darkMode ? 'bg-gray-800 border-amber-500 text-gray-300' : 'bg-amber-50 border-amber-400 text-gray-600'}`}>"{selectedText}"</div>
                        )}

                        {/* Kategori Seçimi Kaldırıldı - Modal sadece seçim için (Çeviri/Yazım Hatası) kullanılacak */}
                        {/* Varsayılan olarak Translation veya Typo category parent'tan gelecek */}
                        ...
                        <div className="relative">
                            <textarea
                                ...
                                className={`w-full p-4 rounded-xl min-h-[120px] outline-none border resize-y mb-2 ${darkMode ? 'bg-gray-800 border-gray-600 text-gray-200 placeholder-gray-500' : 'bg-[#fffcf5] border-[#e6e0d2] text-[#5c4033] placeholder-gray-400'}`}
                            ></textarea>
