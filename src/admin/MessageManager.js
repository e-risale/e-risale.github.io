...
const MessageManager = ({
    ...
    const getFilteredFeedbacks = () => {
        ...
        return feedbacks.filter(fb => {
            ...
            // 4. Uygunsuz / Şikayet (YENİ)
            if (activeTab === 'inappropriate') return (fb.category === 'inappropriate' || fb.category === 'report') && status !== 'archived';

            // 5. İncelenecekler (Okunmuş/İncelenmiş ama henüz onaylanmamış/arşivlenmemiş)
            if (activeTab === 'pending') return status === 'read';

            // 6. Yayındakiler (Onaylanmışlar)
    ...
    // Grouping Logic for Approved Tab
    const groupedThreads = React.useMemo(() => {
        if (activeTab !== 'approved') return { threadList: [], orphans: [] };

        const threads = {};
        const orphans = [];
        const idMap = new Map();

        filteredFeedbacks.forEach(fb => idMap.set(fb.id, fb));

        filteredFeedbacks.forEach(fb => {
            if (fb.parentId) {
                if (idMap.has(fb.parentId)) {
                    if (!threads[fb.parentId]) {
                        threads[fb.parentId] = { parent: idMap.get(fb.parentId), replies: [] };
                    }
                    threads[fb.parentId].replies.push(fb);
                } else {
                    orphans.push(fb);
                }
            } else {
                if (!threads[fb.id]) {
                    threads[fb.id] = { parent: fb, replies: [] };
                }
            }
        });
        ...
        threadList.forEach(t => {
            t.replies.sort((a, b) => (a.date?.toMillis ? a.date.toMillis() : 0) - (b.date?.toMillis ? b.date.toMillis() : 0));
        });

        // Sort Threads by Last Activity (Newest First)
        threadList.forEach(t => {
            const lastReplyDate = t.replies.length > 0 ? t.replies[t.replies.length - 1].date : null;
            const parentDate = t.parent.date;
            const d1 = parentDate?.toMillis ? parentDate.toMillis() : 0;
            const d2 = lastReplyDate?.toMillis ? lastReplyDate.toMillis() : 0;
            t.lastActivity = Math.max(d1, d2);
        });

        threadList.sort((a, b) => b.lastActivity - a.lastActivity);
        orphans.sort((a, b) => (b.date?.toMillis ? b.date.toMillis() : 0) - (a.date?.toMillis ? a.date.toMillis() : 0));

        return { threadList, orphans };
    ...
    const cardActions = {
        ...
        onArchive,
        onDelete,
        onReply: (fb) => { setSelectedFeedback(fb); setAutoFocusReply(true); },
        updateCategory,
        onUpdateContent,
        onFindOriginal: (id) => {
            const f = feedbacks.find(x => x.id === id);
            if (f) setSelectedFeedback(f);
            else alert("Mesaj bulunamadı. ID: " + id);
        }
    };
    ...
    const counts = {
        ...
        inappropriate: feedbacks?.filter(f => (f.category === 'inappropriate' || f.category === 'report') && f.status !== 'archived').length || 0,
    ...
    return (
        <div className={`min-h-screen font-sans transition-colors duration-300 ${darkMode ? 'bg-[#1a1b1e] text-gray-200' : 'bg-gray-50 text-gray-800'}`}>
            ...
            <header className={`sticky top-0 z-30 border-b transition-colors duration-300 ${darkMode ? 'bg-[#25262b] border-gray-700' : 'bg-white border-gray-200'}`}>
                <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <h1 className={`text-2xl font-bold tracking-tight ${darkMode ? 'text-gray-100' : 'text-gray-900'}`}>Mesaj Yönetimi</h1>
                    </div>
            ...
            {/* Tabs */}
            <div className={`fixed top-20 left-0 right-0 z-20 border-b shadow-sm overflow-x-auto transition-colors duration-300 ${darkMode ? 'bg-[#1a1b1e] border-gray-700' : 'bg-white border-gray-200'}`}>
                <div className="max-w-7xl mx-auto px-4 md:px-8 flex gap-4 md:gap-8 pb-3 pt-3">
                    ...
                    <TabButton active={activeTab === 'inappropriate'} onClick={() => setActiveTab('inappropriate')} count={counts.inappropriate} color="red" darkMode={darkMode}>Uygunsuz</TabButton>
            ...
            {/* Content */}
            <div className="pt-48 pb-12 px-2 md:px-8 max-w-7xl mx-auto">
                {isLoading ? (
                    ...
                    <div className="flex flex-col gap-3">
                        {activeTab === 'approved' ? (
                            <>
                                {groupedThreads.threadList.map(thread => (
                                    <div key={thread.parent.id} className={`p-2 rounded-2xl border transition-all ${darkMode ? 'border-gray-700 bg-gray-800/30' : 'border-amber-200 bg-amber-50/50'}`}>
                                        {/* Parent */}
                                        <FeedbackCard
                                            fb={thread.parent}
                                            activeTab={activeTab}
                                            onClick={() => setSelectedFeedback(thread.parent)}
                                            actions={cardActions}
                                            darkMode={darkMode}
                                        ...
                                        {thread.replies.length > 0 && (
                                            <div className="mt-2 pl-4 md:pl-8 space-y-2 border-l-2 border-gray-300/30 dark:border-gray-700/30 ml-4 pb-2">
                                                {thread.replies.map(reply => (
                                                    <div key={reply.id} className="relative">
                                                        <div className="absolute top-4 -left-4 w-4 h-[2px] bg-gray-300 dark:bg-gray-700"></div>
                                                        <FeedbackCard
                                                            fb={reply}
                                                            activeTab={activeTab}
                                                            onClick={() => setSelectedFeedback(reply)}
                                                            actions={cardActions}
                                                            darkMode={darkMode}
                                                            formatDate={formatDate}
                                                            isThreadReply={true}
                                ...
                                {/* Orphans */}
                                {groupedThreads.orphans.length > 0 && (
                                    <div className="mt-8 border-t pt-4">
                                        <h3 className="text-sm font-bold opacity-50 mb-4 px-2">DİĞER (BAĞIMSIZ/SİLİNMİŞ ANA MESAJLAR)</h3>
                                        <div className="flex flex-col gap-3">
                                            {groupedThreads.orphans.map(fb => (
                                                <FeedbackCard
                                                    key={fb.id}
                                                    fb={fb}
                                                    activeTab={activeTab}
            ...
            {selectedFeedback && (
                <FeedbackDetailModal
                    ...
                    onFindOriginal={(id) => {
                        const f = feedbacks.find(x => x.id === id);
                        if (f) setSelectedFeedback(f);
                        else alert("Mesaj bulunamadı (silinmiş veya yüklenmemiş olabilir). ID: " + id);
                    }}
                />
            )}
        </div>
...
const FeedbackDetailModal = ({ fb, onClose, darkMode, formatDate, onMarkRead, onApprove, onArchive, onDelete, onReply, updateCategory, initialShowReply = false, onFindOriginal, onUpdateContent }) => {
    const [isDropdownOpen, setIsDropdownOpen] = useState(false);
    const [showReplyInput, setShowReplyInput] = useState(initialShowReply);
    const [replyText, setReplyText] = useState(fb.reply || "");
    const [editedMessage, setEditedMessage] = useState(fb.feedback || "");

    // Determine type
    const isBug = fb.category === 'bug' || fb.category === 'typo';
    const isReport = fb.category === 'inappropriate' || fb.category === 'report';
    const isPrivate = fb.isPrivate; // Check Private

    const handleSendReply = async () => {
        // Update Original Message if changed
        if (editedMessage !== fb.feedback) {
            let finalMsg = editedMessage.trim();

            // Remove old disclaimer if exists
            finalMsg = finalMsg.split(/\n\n\(Editör tarafından düzeltilmiştir/)[0];

            // Add new disclaimer with date
            const now = new Date();
            const dateStr = now.toLocaleDateString('tr-TR', { day: '2-digit', month: '2-digit', year: 'numeric' });
            const timeStr = now.toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit' });

            finalMsg += `\n\n(Editör tarafından düzeltilmiştir. ${dateStr} - ${timeStr})`;

            if (onUpdateContent) {
                await onUpdateContent(fb.id, finalMsg);
            }
        }
            ...
            <div className={`w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl shadow-2xl p-6 relative ${darkMode ? 'bg-[#1a1b1e] border border-gray-700' : 'bg-white'}`} onClick={e => e.stopPropagation()}>
                ...
                <div className="flex items-center gap-3 mb-6 mr-8"> {/* Added mr-8 to avoid overlap with Close button */}
                    ...
                    <div className="flex-1">
                                ...
                                <div className="flex items-center gap-1 pl-2 border-l border-dashed border-gray-300 dark:border-gray-700">
                                    ...
                                    {fb.status !== 'approved' && fb.status !== 'archived' && (
                                        <>
                                            ...
                                            {!isBug && !isReport && !isPrivate && <ActionButton onClick={() => { onApprove(fb.id); onClose(); }} icon="✅" color="green" title="Yayınla" darkMode={darkMode} />}
                                            {isReport && fb.reportedCommentId && <ActionButton onClick={() => onFindOriginal(fb.reportedCommentId)} icon="🔍" color="amber" title="İlgili Mesaja Git" darkMode={darkMode} />}
                        ...
                        {isPrivate && (
                            <div className="mt-1">
                                <span className="inline-flex items-center gap-1 font-bold text-xs text-indigo-500 bg-indigo-100 dark:bg-indigo-900/30 px-2 py-1 rounded">
                                    🔒 GİZLİ MESAJ (Yayınlanmaz)
                                </span>
                            </div>
                        )}
                    </div>
                ...
                <div className="space-y-4">
                    ...
                    {fb.context && (
                        <div className={`mt-2 mb-4 p-3 rounded-lg text-sm opacity-90 border leading-relaxed ${darkMode ? 'bg-gray-800/50 border-gray-700 text-gray-400' : 'bg-gray-100 border-gray-200 text-gray-600'}`}>
                            <div className="font-bold text-xs uppercase mb-1 opacity-70">BAĞLAM/KONUM:</div>
                            <div className="font-serif italic">{fb.context}</div>
                        </div>
                    )}
                    ...
                    {showReplyInput ? (
                        <div className="space-y-2 animate-in fade-in">
                            <div className="text-xs font-bold text-amber-600">Mesaj İçeriğini Düzenle:</div>
                            <textarea
                                value={editedMessage}
                                onChange={(e) => setEditedMessage(e.target.value)}
                                className={`w-full p-4 rounded-xl text-lg leading-relaxed outline-none border focus:ring-2 focus:ring-amber-500 transition-all ${darkMode ? 'bg-[#25262b] text-gray-200 border-gray-600' : 'bg-white text-gray-800 border-gray-300'}`}
                                rows={8}
                            />
                        </div>
                    ) : (
...
const FeedbackCard = ({ fb, onClick, actions, darkMode, formatDate }) => {
    const { onMarkRead, onApprove, onArchive, onDelete, onReply, updateCategory, onFindOriginal } = actions;
    ...
    const catLabel = isBug ? '[HATA]' : isSuggestion ? '[ÖNERİ]' : isReport ? '[UYGUNSUZ]' : '[YORUM]';
    const catClass = isBug || isReport ? 'text-red-500' : isSuggestion ? 'text-green-600' : 'text-blue-500';
            ...
            <div className="flex items-center gap-3 shrink-0 w-64">
                ...
                <div className="min-w-0 flex-1">
                    ...
                    <div className="flex items-center gap-2 text-xs">
                        ...
                        {/* New Private Badge */}
                        {isPrivate && (
                            <span className="font-bold text-indigo-500 flex items-center gap-1 bg-indigo-100 dark:bg-indigo-900/30 px-1 rounded">
                                🔒 GİZLİ
                            </span>
                        )}
            ...
            <div className="flex items-center gap-1 shrink-0 pl-2" onClick={e => e.stopPropagation()}>
                ...
                {fb.status !== 'approved' && fb.status !== 'archived' && (
                    <>
                        ...
                        {!isBug && !isReport && !isPrivate && <ActionButton onClick={() => onApprove(fb.id)} icon="✅" color="green" title="Yayınla" darkMode={darkMode} />}
                        {isReport && fb.reportedCommentId && <ActionButton onClick={() => onFindOriginal(fb.reportedCommentId)} icon="🔍" color="amber" title="İlgili Mesaja Git" darkMode={darkMode} />}
